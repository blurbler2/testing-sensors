...
HAL_StatusTypeDef bme280_init(I2C_HandleTypeDef *hi2c, uint16_t addr)
{
  ...
  // ctrl_hum x1 (must be set BEFORE reading humidity calibration and ctrl_meas)
                                          ...
                                          float *temperature, uint32_t *pressure, float *humidity)
{
  ...
  *pressure = comp_P_out(adc_P) / 256; // Q24.8 → Pa
  *humidity = (float)comp_H_out(adc_H) / 1024.0f;
  return HAL_OK;
}
